...
class Agent:
    ...
    def calc_param(self):
        ...
        for index in self.envir.spr_df.index:
            ...
            #print (price_hist_temp_df.head(1))
            ...
            price_hist_temp_df = price_hist_temp_df.reset_index()
            price_hist_prev_df = price_hist_temp_df.copy()
            price_hist_prev_df.index += self.year_days
            min_date = price_hist_temp_df.loc[min(price_hist_temp_df.index)]
            max_date = price_hist_temp_df.loc[max(price_hist_temp_df.index)]
            price_hist_temp_df = pd.merge(price_hist_temp_df, price_hist_prev_df[['index', 'date','price']], how = 'outer', left_index = True, right_index=True, suffixes=('', '_prev'))

            price_hist_temp_df['index_prev'][:self.year_days] = min_date['index']
            price_hist_temp_df['date_prev'][:self.year_days] = min_date['date']
            price_hist_temp_df['price_prev'][:self.year_days] = min_date['price']
            price_hist_temp_df['index'][-self.year_days:] = max_date['index']
            price_hist_temp_df['date'][-self.year_days:] = max_date['date']
            price_hist_temp_df['price'][-self.year_days:] = max_date['price']
            price_hist_temp_df['diff'] = (price_hist_temp_df['index'] - price_hist_temp_df['index_prev']) / self.year_days
            price_hist_temp_df = price_hist_temp_df[price_hist_temp_df['diff'] > 0]
            price_hist_temp_df = price_hist_temp_df[['date', 'price', 'date_prev', 'price_prev', 'diff']]
            ...
            price_hist_temp_df['res'] = (price_hist_temp_df['price']/price_hist_temp_df['price_prev'])**(1/price_hist_temp_df['diff'])
            price_hist_temp_df['res'] = price_hist_temp_df['res'].astype(float)
            price_hist_temp_df['power'] = price_hist_temp_df.index/self.den_pow
            price_hist_temp_df['weight'] = (1/2**price_hist_temp_df['power'])**price_hist_temp_df['diff']
            ...
            print('index: ', index, 'gm_w: ', gm_w, 'total_1: ', total_1, 'st_dev_w: ', st_dev_w, 'total_2: ', total_2)
