...
class Superviser:
    ...
    Attributes:
        ...
        type_insrt:    Тип инстурмента инвестиций
    """
    
    envir = None       # Окружающая среда
    agent = None       # Агент
    
    def __init__(self, envir, agent, type_insrt):
    ...
    def show_port(self):
        ...
        port_temp_df = pd.merge (self.envir.port_df, self.envir.spr_df, on = ['insrt_id'])
        port_temp_df = pd.merge (port_temp_df, self.envir.price_df, on = ['insrt_id'])
        port_temp_df['value_insrt'] = port_temp_df['quantity'] * port_temp_df['price']
        port_temp_df['value_insrt'] = port_temp_df['value_insrt'].astype(int)
        ...
        port_temp_df = port_temp_df[['insrt_id', 'manager', 'insrt', 'value_insrt', 'value', 'part']]
        return port_temp_df
